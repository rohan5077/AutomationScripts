(function() {
    ...
    function renderZonePivots(insertAfter, groups, definitions, preferences) {
        ...
        alert.addEventListener("change", storePreference);
    }
    ...
    var LocalProgressService = function() {
        function LocalProgressService(localStorage, unitUid) {
        ...
        LocalProgressService.prototype.completeUnit = function(unitUid) {
            var _this = this;
            ...
            return this.moduleUidLoaded.then(function(moduleUid) {
                ...
                var freshStart = {
                    moduleUid: moduleUid,
                    progress: (_a = {}, _a[unitUid] = true, _a)
                };
                if (!data || data.moduleUid !== moduleUid) {
                    data = freshStart;
                } else {
                    data.progress[unitUid] = true;
                }
                _this.localStorage.setItem(_this.key, JSON.stringify(data));
                eventBus.publish(new ProgressUpdatedEvent(data.progress));
                return Promise.resolve();
            });
        };
    ...
    var ProgressUpdatedEvent = function() {
        function ProgressUpdatedEvent(moduleProgress) {
    ...
    function moduleUnitPage() {
        ...
        progressService = new LocalProgressService(localStorage, unitId);
        renderInteractiveElement();
        currentModule.then(function(module) {
            renderModuleNavigation(unitId, module).then(function(navItems) {
                if (navItems) {
                    navigationItems = navItems;
                }
                eventBus.subscribe(ProgressUpdatedEvent, function(event) {
                    updateNavigationItems(navigationItems, event.moduleProgress);
                });
                switch (completionType) {
                  case "view":
                    return progressService.completeUnit(unitId);
                    break;
    ...
    function renderModuleNavigation(unitId, module) {
        return contentLoaded.then(function() {
            ...
            buildNextCTA(module.units, unitIndex, unitTotal);
            return Array.from(document.querySelectorAll(".module-navigation-item"));
        }).catch(function(e) {
            return [];
        });
    }
    ...
    function buildUnitMenu(module, unitIndex) {
        ...
        document.querySelector(".has-centered-menu li:nth-of-type(" + (unitIndex + 1) + ")").classList.add("is-current");
    }
    function buildNextCTA(unit, unitIndex, unitTotal) {
        if (unitIndex !== unitTotal) {
            var nextSection = document.getElementById("next-section");
            var nextMconstext = document.getElementById("next-module");
            var continueButton = document.getElementById("continue");
            continueButton.href = "/" + msDocs.data.userLocale + unit[unitIndex + 1].url;
            nextMconstext.innerText = loc.next + ": " + unit[unitIndex + 1].title;
            nextSection.classList.remove("is-invisible");
        }
    }
    ...
    function updateNavigationItems(navigationItems, moduleProgress) {
        navigationItems.forEach(function(item) {
            item.dataset.navUid in moduleProgress ? item.classList.add("is-complete") : item.classList.remove("is-complete");
        });
    }
    ...
    function tutorialPage() {
        contentLoaded.then(function() {
            ...
            progressService$1 = new LocalTutorialProgressService(localStorage$2, "section_" + location.pathname.replace(/[^a-zA-Z\d\s]+/g, "_"));
            processQueryString();
    ...
    function showStep(step) {
        ...
        stopVideo();
        progressService$1.getProgress().then(renderToc);
        progressService$1.setProgress(step);
        var isFirst = step === 0;
